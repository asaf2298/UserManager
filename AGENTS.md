...
### What this is
Vecret ("Personal", formerly Esay) is a **Vercel serverless proxy/aggregator for Stremio & Kodi addons**. Pure JavaScript (ES modules, `"type": "module"`), Node.js runtime, single runtime dependency (`node-fetch`). Each file under `api/*.js` is a serverless handler with the Vercel Node signature `export default async function handler(req, res)`. Shared logic lives in `lib/`. Stream-time ID mapping uses optional **Supabase** tables (`personal_*`); when disabled, behavior is unchanged. There is **no build step**.
...
`USER_CONFIGS` is a JSON object keyed by the URL path segment (`<USER_KEY>`); `profile` / `name` configure ranking quotas and display. Catalog/meta for IMDb ids use public Cinemeta; Kan-Box catalogs (live + IL VOD) appear in Discovery/Board. `TV_ADDON_URL`, `TMDB_API_KEY` are optional. Put Yastream in `ADDON_URLS` for `tt` streams and `kisskh:` / `idrama:` / `onetouchtv:` provider ids.
