...
/**
 * Map Stremio episode within a stored compact range row.
 * Row: from_e..to_s (inclusive) → target starts at to_e.
 * @returns {number | null}
 */
export function mapEpisodeInStoredRange(episode, row) {
  if (!row || episode === undefined || episode === null) return null;
  const ep = Number(episode);
  if (!Number.isFinite(ep)) return null;
  const start = Number(row.from_e);
  const end = row.to_s != null ? Number(row.to_s) : start;
  const tgtStart = Number(row.to_e);
  if (!Number.isFinite(start) || !Number.isFinite(tgtStart)) return null;
  if (ep < start || ep > end) return null;
  return tgtStart + (ep - start);
}
