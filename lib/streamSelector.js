 ...
 *
 * Takes gain as a separate argument rather than a candidate-with-gain-merged
 * object (#60): takeOne used to spread two new objects per comparison just to
 * attach `gain` -- ~100,000 allocations on a 100-target `kodi` request scanning
 * a 500-candidate pool. Pure performance; comparison order is unchanged.
 */
function compareForSelection(a, gainA, b, gainB) {
...
export function selectStreams(representatives, profile, trustSnapshot) {
  ...
  const vipCandidates = pool
    ...
    .sort((a, b) => compareForSelection(a, a.baseScore, b, b.baseScore));
  ...
  const takeOne = (predicate) => {
    ...
    for (const candidate of pool) {
      ...
      const gain = marginalGain(candidate, counters, profile);
      if (!best || compareForSelection(candidate, gain, best.candidate, best.gain) < 0) {
        best = { candidate, gain };
      }
    }
