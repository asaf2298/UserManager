...
export function buildAuditRows(candidates, context) {
  ...
  for (const candidate of candidates) {
    ...
    // Debrid resolve links (Comet/Torrentio/etc.) often bury the infoHash in
    // the playable URL even when the stream object omits it -- fall back the
    // same way lib/streamSighting.js already does, or every such candidate is
    // silently dropped before it ever reaches TorBox verification.
    const infoHash = String(
      resolveTorrentIdentity({ infoHash: candidate.stream?.infoHash, playableUrl: candidate.stream?.url }).infoHash || '',
    ).toLowerCase();
