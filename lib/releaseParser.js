...
const RELEASE_GROUP_RE = /(?:^|[\s.\-_])-\s*([A-Za-z0-9][A-Za-z0-9._]{1,24})\s*(?:\.[a-z0-9]{2,4})?\s*$/;
...
function detectReleaseGroup(rawTitle) {
  ...
  for (const line of lines) {
    ...
    if (bracket) return bracket[1].toLowerCase().replace(/\s+/g, '');
    const dashed = line.match(RELEASE_GROUP_RE);
    if (dashed) return dashed[1].toLowerCase().replace(/\.$/, '');
  }
  return null;
}
...
function detectSeeders(text, provider) {
  const trusted = !provider || provider.trustedFields?.includes('seeders');
  const match = text.match(SEEDERS_RE);
...
export function parseRelease(stream, context = {}) {
  ...
  const seeders = detectSeeders(text, provider);
