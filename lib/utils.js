...
export const REGEX_DOWNLOAD = /\[[^\]]*(download|⬇️)[^\]]*\]/gi;
...
export function isVIPSource(stream) {
  ...
  let vip = sourceUrl.includes('kan-box-addon.vercel.app') || sourceUrl.includes('animeil');
  
  // תיקון דרגון בול: אם זה חיפוש פרק, אבל ה-VIP החזיר סרט, מורידים לו את הפריבילגיה
  const text = getTextForAnalysis(stream);
  if (vip && stream._isEpisodeQuery && text.includes('סרט') && !text.includes('פרק')) {
      vip = false;
  }
...
export function getWeightTiers(streams) {
  for (const s of streams) {
    ...
    let tier = 0;
    if (res === 4) {
      if (size >= 25) tier = 3; else if (size >= 12) tier = 2; else if (size >= 5) tier = 1;
    } else if (res === 3) {
      if (size >= 8) tier = 3; else if (size >= 3.5) tier = 2; else if (size >= 1.5) tier = 1;
    } else if (res === 2) {
      if (size >= 3) tier = 3; else if (size >= 1.5) tier = 2; else if (size >= 0.5) tier = 1;
    } else {
      if (size >= 1.5) tier = 3; else if (size >= 0.5) tier = 2; else if (size >= 0.2) tier = 1;
    }
    s._weightTier = tier;
  }
}
...
export function getVisualWeight(stream) {
...
export function getQualityScoreForPreSort(stream) {
