...
export const REGEX_DOWNLOAD = /\[[^\]]*(download|uncached|⬇️|⏳)[^\]]*\]/gi;
...
export function isCached(stream) {
  ...
  // חסימה אגרסיבית: זיהוי כל אות של Uncached / הורדה / שעון חול
  const isUncachedSignal = /uncached|⏳|download|⬇️|downloading|download to debrid/i.test(text);

  if (isUncachedSignal) {
    res = false; // ברגע שיש סימן של uncached - זה בוודאות לא ב-Cache!
  } else {
...
export function isVIPSource(stream) {
  ...
  // מחזירים את ההתנהגות המקורית: אם זה הגיע מהמקורות האלה, זה תמיד VIP
