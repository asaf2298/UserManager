 ...
 */
export const REGEX_UNCACHED_SIGNAL = /un[\s._-]?cached|not[\s._-]?cached|⏳|⌛|⬇️|⬇|downloading|\bdownload\b|download[\s._-]?to[\s._-]?debrid|add[\s._-]?to[\s._-]?debrid|to[\s._-]?debrid|nc[\s._-]?torrent|\bnc\b|\[dl\]|\(dl\)|instant[\s:=_-]?false|cached[\s:=_-]*\s*(?:no|false|0)/i;
