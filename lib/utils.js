 ...
 */
export const REGEX_NOTICE_STREAM = /\bno (?:streams?|playable streams?) (?:were )?found\b|\bno direct http rows? (?:were|was) available\b|\bcould not load streams?\b|\btry another source,?\s*episode,?\s*or playback option\b|you must sign in\b|sign[\s._-]?in (?:is )?required|authentication is (?:missing|invalid|revoked)|\b(?:monthly|server|hosting) costs? (?:is|are)?\s*covered\b|\bdonor token\b|please\s+(?:donate|consider donating)|select this stream to see how to sign in|\/stream-errors\//i;
...
export function isVIPSource(stream) {
  ...
  const text = getTextForAnalysis(stream);
  const vipBrand = /\btele(?:gram|fram)\b|your[\s._-]?media/i.test(text);
