...
export function isVIPSource(stream) {
  ...
  // Content-prefix VIP: Telegram / Your Media branded streams (name, title, description)
  // Matches "telegram", "telefram" typo variants, "your media", "yourmedia", "your-media".
  // Genuine Telegram/Your-Media VIP sources are always plain single-hop HTTP links, so
  // restrict the text match to that shape — otherwise an incidental "Telegram"/"Your Media"
  // mention inside a normal torrent/usenet release filename (e.g. an uploader credit, common
  // on regional/Telegram-ripped content re-uploaded to indexers) would hijack an otherwise
  // correctly cached/uncached torrent or usenet item into showing as VIP/direct-web.
  ...
  const text = getTextForAnalysis(stream);
  const vipBrand = isPlainHttpLink && /\btele(?:gram|fram)\b|your[\s._-]?media/i.test(text);
...
export function getWeightTiers(streams) {
  for (const s of streams) {
    ...
    let tier = 0;
    if (res === 4) {
      if (size >= 25 && size <= 150) tier = 3; else if (size >= 12) tier = 2; else if (size >= 5) tier = 1;
    } else if (res === 3) {
