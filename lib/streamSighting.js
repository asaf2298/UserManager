...
export function extractStreamIdentity(candidate) {
  ...
  return {
    playableUrl: stream.url || stream.externalUrl || null,
    infoHash: normalizeHash(stream.infoHash),
 ...
 */
export async function resolvePlayableSighting({ contentId, identity }) {
  if (!canRead() || !contentId) return { found: false, strength: MATCH_STRENGTH.NONE, reason: 'unavailable' };

  const params = new URLSearchParams({
    select: 'playable_url,video_hash,filename,video_size,info_hash,file_idx,provider_id,seen_at',
...
function ok(row, strength) {
  return {
    ...
    fileIdx: Number.isFinite(Number(row.file_idx)) ? Number(row.file_idx) : null,
