 ...
 *   ID_RESOLVE_MAX_EXTRA_FETCHES (default 7) — cap total extra addon requests
...
import { canRead, selectRows } from './supabaseServer.js';

const DEFAULT_TIMEOUT_MS = 400;
const DEFAULT_MAX_EXTRA_FETCHES = 7;
...
async function fetchTitleByImdb(imdbId, timeoutMs) {
  ...
  const rows = await selectRows('personal_titles', params, timeoutMs);
  if (rows == null) {
    console.warn('[ID-RESOLVE] lookup failed (personal_titles)');
    return null;
  }
...
async function fetchEpisodeMaps(imdbId, season, episode, timeoutMs) {
  ...
  const rows = await selectRows('personal_episode_map', params, timeoutMs);
  if (rows == null) {
    console.warn('[ID-RESOLVE] lookup failed (personal_episode_map)');
    return {};
  }
  if (!rows.length) {
    ...
    const exactRows = await selectRows('personal_episode_map', exactParams, timeoutMs);
...
async function fetchAliasRows(imdbId, timeoutMs) {
  ...
  const rows = await selectRows('personal_akas', params, timeoutMs);
  if (rows == null) {
    console.warn('[ID-RESOLVE] lookup failed (personal_akas)');
    return [];
  }
  return rows;
}
...
export async function resolveContentContext(stremioType, rawIdWithExt) {
  if (!isIdResolveEnabled() || !canRead()) return null;
