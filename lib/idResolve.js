...
function aliasUsefulnessScore(row, preferredLangs = []) {
  ...
  // Prefer cleaner native titles: pure-script JP, no year suffixes, no arc subtitles
  if (lang === 'ja') {
    const jpChars = (title.match(/[\u3040-\u30ff\u3400-\u9fff\uff66-\uff9d]/g) || []).length;
    if (jpChars / Math.max(title.length, 1) >= 0.7) score += 20;
  }
  if (/\(\d{4}\)/.test(title)) score -= 25;
  if (/^\(\d{4}\)/.test(title)) score -= 25;
  if (title.includes('-') && lang === 'ja') score -= 10;
