...
export function planAliasTextFanout(addons, context, excludeTitles = []) {
  ...
  const titles = pickAliasSearchTitles(context.aliasRows || [], excludeTitles, aliasPickOpts(context));
  return buildAliasFanoutPlan(addons, titles[0]);
}
...
export function planDeferredAliasTextFanout(addons, context, excludeTitles = []) {
  if (!isIdResolveAliasEnabled() || !context?.isAnimeContent) return [];
  if (maxAliasSearches() < 2) return [];
  const titles = pickAliasSearchTitles(context.aliasRows || [], excludeTitles, aliasPickOpts(context));
  return buildAliasFanoutPlan(addons, titles[1]);
}
