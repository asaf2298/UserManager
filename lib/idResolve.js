...
/** Prefer display + ASCII Latin/romaji titles; demote foreign-script / year-prefixed junk for torrent search. */
function aliasUsefulnessScore(row) {
  const title = String(row.title || '').trim();
  let score = Number(row.weight) || 0;
  if (row.kind === 'display') score += 50;
  ...
  else if (ratio >= 0.55) score += 15; // Latin with diacritics — weaker for torrent indexes
...
/**
 * Top alias titles for conservative text search. Excludes duplicates and TMDB primary when known.
 * If the Manami display title is already covered by TMDB text search, returns [] — do not
 * fall through to random foreign-language synonyms (e.g. Croatian Death Note).
 * @param {object[]} aliasRows — from personal_akas
 * @param {string[]} excludeTitles — titles to skip (e.g. TMDB primary)
 */
export function pickAliasSearchTitles(aliasRows, excludeTitles = []) {
  if (!aliasRows?.length) return [];
  const exclude = new Set(excludeTitles.map(normalizeSearchTitle).filter(Boolean));

  const display = aliasRows.find(r => r.kind === 'display' && String(r.title || '').trim());
  if (display) {
    const displayKey = normalizeSearchTitle(display.title);
    if (exclude.has(displayKey)) return [];
  }
  ...
  for (const row of aliasRows) {
    ...
    // Skip ultra-short abbreviations (DN, FMA) — weak for torrent search
    if (title.length < 6 && !/\s/.test(title)) continue;
    seen.add(key);
    candidates.push({ title, score: aliasUsefulnessScore(row) });
  }

  candidates.sort((a, b) => b.score - a.score || a.title.localeCompare(b.title));
  return candidates.slice(0, maxAliasSearches()).map(c => c.title);
}
 ...
 */
/**
 * Alias text searches for anime-capable addons only. Empty when alias flag off or not anime.
 * @returns {{ url: string, searchTitle: string }[]}
 */
export function planAliasTextFanout(addons, context, excludeTitles = []) {
  if (!isIdResolveAliasEnabled() || !context?.isAnimeContent) return [];
  const titles = pickAliasSearchTitles(context.aliasRows || [], excludeTitles);
