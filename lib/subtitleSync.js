 ...
 *   - Up to four `סנכרן כתבויות` tracks appear in the *first* subtitle list.
...
/** Hebrew label shown in the player's subtitle list. */
export const SYNC_TRACK_LABEL = 'סנכרן כתבויות';
  ...
  bases.forEach((base, baseIndex) => {
    ...
    for (const slot of REFERENCE_SLOTS) {
      ...
      tracks.push({
        ...
        lang: 'heb',
        title: `${SYNC_TRACK_LABEL} · ${baseIndex + 1}/${slot === REFERENCE_SLOT.OFFICIAL ? 'מקור' : 'EN'}`,
        _syncTrack: true,
        _syncSlot: slot,
