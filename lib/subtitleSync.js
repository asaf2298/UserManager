...
export const SYNC_MESSAGES = {
  ...
  TRY_OTHER: 'try another sync subtitle',
 ...
 *
 * providerId/providerFamily/stratum are carried from the resolved sighting so
 * the worker's ffprobe result can later be attributed to a provider for the
 * `integrity` trust metric. All three are optional/nullable: a miss just means
 * that job's probe result cannot be graded, not that the job itself fails.
 */
export async function enqueueSyncJob({
  videoKey, subFingerprint, slot, baseSubUrl, contentType, contentId, playableUrl, infoHash, fileIdx,
  providerId = null, providerFamily = null, stratum = null,
}) {
...
export async function resolveSyncResponse({
  ...
  if (!job) {
    ...
    await enqueueSyncJob({
      ...
      providerId: sighting.providerId,
      providerFamily: sighting.providerFamily,
      stratum: sighting.stratum,
    });
  }
