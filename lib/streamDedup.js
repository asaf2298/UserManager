...
const MINHASH_SEEDS = (() => {
  const digest = crypto.createHash('sha256').update(`minhash|${PARSER_VERSION}`).digest();
  const seeds = new Array(MINHASH_VALUES);
  for (let i = 0; i < MINHASH_VALUES; i++) {
    seeds[i] = digest.readUInt32BE((i * 4) % (digest.length - 4));
  }
  return seeds;
})();
...
export function duplicateVerdict(a, b) {
  ...
  if (hashA && hashB && hashA === hashB) {
    if (a.features.release.fileFingerprint === b.features.release.fileFingerprint) {
      return { duplicate: true, rule: 'infohash_and_file', similarity: 1 };
    }
    // Same torrent, different selected file: two different episodes, keep both.
...
export { minhashSignature, normalizedHash, sizeRatio };
