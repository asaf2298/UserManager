 ...
 *   - "Cached" comes from provider capability rules; VIP is host-authoritative
 *     or the global telegram_bot / your-media title prefix on direct HTTP links.
 */
...
export const MATCH_ELIGIBILITY_THRESHOLD = 0.70;
...
function computeSizePlausibility(release, runtimeMinutes) {
  ...
  const x = Math.log2(bitrateMbps / expected);
...
function computeLanguageRelevance(release, { matchLevel, originalLanguage }) {
  ...
  if (langs.has('he')) {
    ...
    if (strongMatch) return { value: 0.85, reason: 'hebrew_tag_confirmed' };
    return { value: 0.85, reason: 'hebrew_tag' };
  }
...
function computeCredibility(release) {
  ...
  for (const reason of reasons) {
    ...
    if (penalty !== undefined && penalty < value) value = penalty;
  }
...
export { MATCH_CONFIDENCE, RESOLUTION_BASE, SOURCE_ADJUST, EXPECTED_BITRATE, hasValidHttpLocator };
