...
import { getExtraStreamIds, planExtraStreamFanout } from './idResolve.js';
 ...
 */
function finalizeStreams(rawStreams, { isEpisode, sizeLimitGB, minSeedersUncached, releaseYear, maxResults, type }) {
...
export async function fetchAndSortStreams(type, idWithExt, context) {
  const {
    ...
    queryHint = '', maxResults = Infinity, idResolveContext = null
  } = context;
  ...
  const extraStreamIds = getExtraStreamIds(idResolveContext);
