...
// #59: with a cap of 2 and 4 query entries per provider (canonical + primary +
// he/ru extra-language), the 3rd/4th wave only started once the first two
// entries in the per-provider slot fully finished -- pushing later waves
// (Russian, in the common case) past PER_REQUEST_DEADLINE_MS before they ever
// got a socket. Raised so both extra-language waves fit inside 2 concurrent
// slots per provider.
const PER_PROVIDER_INFLIGHT_CAP = 3;

/** Hard per-upstream deadline, independent of the overall cutoff. */
const PER_REQUEST_DEADLINE_MS = 2800;

/** Budget reserved for parsing, scoring, selection, and serialization. */
const RANKING_RESERVE_MS = 1500;

/**
 * Metadata is an enrichment, never a gate (#59): on a slow-TMDB request the
 * old unbounded await could cost 6.5-9.5s against a 5.5s cutoff, so the
 * `immediate` phase never got submitted before its own deadline and the whole
 * request silently collapsed to canonical-ID only -- no text search, no
 * language fallback, Hebrew included.
 */
...
/**
 * he-first ordering within a query mode (#59): this audience is Hebrew-first,
 * so Hebrew should be guaranteed a scheduler slot rather than winning or losing
 * purely on which provider happened to answer earliest.
 */
const LANG_RANK = { he: 0, en: 1, original: 2, ru: 3 };

/** Lower sorts first. Query mode dominates; language only breaks ties within it. */
...
class RetrievalScheduler {
  ...
  submit(providerKey, taskFactory, priority = 0) {
    const settled = new Promise((resolve) => {
      this.queue.push({ providerKey, taskFactory, resolve, priority });
    });
  ...
  #drain() {
    while (this.globalActive < GLOBAL_INFLIGHT_CAP) {
      // Priority-ordered, not FIFO (#59): a per-provider slot freeing up must go
      // to the highest-priority *waiting* entry (e.g. Hebrew), not whichever
      // entry happened to queue first -- otherwise which language wins is a
      // function of provider answer latency instead of query-mode/language rank.
      let bestIndex = -1;
      for (let i = 0; i < this.queue.length; i++) {
        const item = this.queue[i];
        if ((this.perProvider.get(item.providerKey) || 0) >= PER_PROVIDER_INFLIGHT_CAP) continue;
        if (bestIndex === -1 || item.priority < this.queue[bestIndex].priority) bestIndex = i;
      }
 ...
 */
/** Which of meta's language fields a given search title text came from. */
function titleLangKey(title, contentMeta) {
  if (!contentMeta) return 'en';
  if (title === contentMeta.he) return 'he';
  if (title === contentMeta.ru) return 'ru';
  if (title === contentMeta.original && contentMeta.original !== contentMeta.en) return 'original';
  return 'en';
}
  ...
  if (primaryTitle) {
    for (const { url, provider } of providers) {
      ...
      plan.push({
        ...
        langKey: titleLangKey(primaryTitle, contentMeta),
        phase: 'immediate',
      });
    }
  }
  ...
  for (const title of titles.slice(1)) {
    for (const { url, provider } of providers) {
      ...
      plan.push({
        ...
        searchTitle: title,
        langKey: titleLangKey(title, contentMeta),
        phase: 'immediate',
      });
    }
  }
...
async function executePlanEntry(entry, { type, clientUA, clientIp, deadlineAt, requestStartedAt }) {
  ...
  try {
    ...
    if (noticeCount > 0) {
      console.log(`[PERSONAL STREAM] 🧹 סוננו ${noticeCount} שורות הודעה/פרסום (לא תוכן) ממקור: ${cleanBaseUrl}`);
    }
    return rows;
  } catch (err) {
    return [];
...
export function prunePriority(candidate) {
  ...
  // Profile-neutral (#60): the old 10*V made this a 4K filter -- exactly
  // backwards for the availability-weighted profiles (friends_light weights V
  // at 14/100, F at 26/100), which above MAX_RANKER_CANDIDATES had this prune
  // discard precisely the cached-720p rows those profiles rank highest.
  return (features.V * 3)
    + (features.F * 3)
    + (features.X * 2)
    + (features.D * 1)
...
export function rankAndSelect(rawRows, context) {
  ...
  const { selected, stats: selectStats } = selectStreams(representatives, profile, context.trustSnapshot);
...
export async function retrieveRankAndSelect(type, idWithExt, context) {
  ...
  // Race, don't await: proceed with whatever resolved rather than let a slow
  // TMDB request block the immediate phase past its own deadline (#59).
  const [contentMeta, resolvedCtx] = await Promise.all([
    Promise.race([metaPromise, new Promise(resolve => setTimeout(() => resolve(null), META_BUDGET_MS))]),
  ...
  console.log(
    `[PERSONAL STREAM] ⏱️ cutoff=${cutoff}ms elapsed=${Date.now() - startedAt}ms` +
