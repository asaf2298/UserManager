 ...
 */
function pruneEligibleCandidates(candidates) {
  if (candidates.length <= MAX_RANKER_CANDIDATES) {
    return { candidates, pruned: 0 };
  }

  const streamToCandidate = new Map();
  for (const candidate of candidates) streamToCandidate.set(candidate.stream, candidate);

  const { rows, pruned } = pruneCandidates(candidates.map(c => c.stream));
  return {
    candidates: rows.map(stream => streamToCandidate.get(stream)).filter(Boolean),
    pruned,
  };
}
...
/**
 * Run a retrieval plan against the scheduler: immediate entries now, deferred
 * (2nd alias) only when the shared collection is still thin mid-window.
 *
 * `collected` is a shared mutable array owned by the caller so t=0 canonical
 * rows that arrive during the deferred wait still count toward thinness.
 *
...
export async function executeRetrievalPlan(plan, {
  ...
  for (const entry of plan) {
    ...
    // Canonical IDs may already have been submitted by the caller for t=0 fan-out;
    // skip duplicates when the plan is executed after an early canonical wave.
    if (entry._alreadySubmitted) continue;
    submitEntry(entry);
  }
  ...
  return { dropped: scheduler.dropped, planned: plan.length };
}
...
export async function retrieveRankAndSelect(type, idWithExt, context) {
  ...
  const collected = [];
  const pushEarly = (rows) => {
    if (Array.isArray(rows) && rows.length) collected.push(...rows);
  };

  // t=0: original canonical-ID requests to every provider (plan §3.1).
  const providers = addons.map(url => ({ url, provider: resolveProvider(url, { configured: true }) }));
  for (const { url, provider } of providers) {
    scheduler
  ...
  // Mark canonical entries already launched at t=0 so executeRetrievalPlan
  // does not double-fetch them.
  for (const entry of plan) {
    if (entry.queryMode === QUERY_MODE.CANONICAL_ID) entry._alreadySubmitted = true;
  }

  const { dropped, planned } = await executeRetrievalPlan(plan, {
    type, clientUA, clientIp, deadlineAt, startedAt, scheduler, collected,
  });
