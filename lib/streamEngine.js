...
import { getExtraStreamIds } from './idResolve.js';
...
export async function fetchAndSortStreams(type, idWithExt, context) {
  ...
  if (extraStreamIds.length > 0) {
    console.log(`[ID-RESOLVE] extra fan-out (additive): ${extraStreamIds.join(', ')}`);
    for (const extraId of extraStreamIds) {
      promises = promises.concat(
        addons.map(url => fetchFromAddon(url, `${extraId}.json`))
      );
    }
  }
