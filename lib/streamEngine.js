...
export async function fetchAndSortStreams(type, idWithExt, context) {
  ...
  const deferredAliasFanout = planDeferredAliasTextFanout(addons, resolvedCtx, titles);
  ...
  if (needsDeferredAlias) {
    ...
    console.log(
      `[ID-RESOLVE] deferred alias text search (thin after ${INITIAL_WAIT_MS}ms, safe=${earlySafe.length}/${targetCount}):` +
        ` "${deferredTitle}" on ${[...new Set(deferredAliasFanout.map(p => host(p.url)))].join(', ')}`
    );
    ...
    if (aliasBudget >= 1200) {
      ...
      console.log(`[ID-RESOLVE] after deferred alias "${deferredTitle}" — raw streams=${allStreams.length}`);
    } else {
