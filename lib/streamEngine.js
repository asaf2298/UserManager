...
export async function retrieveRankAndSelect(type, idWithExt, context) {
  ...
  // Deferred entries (the 2nd-ranked alias synonym) only join the fan-out if the
  // immediate results still look thin partway through the collection window —
  // otherwise they would just double anime-addon load with no benefit.
