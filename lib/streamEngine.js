// lib/streamEngine.js
// אגנוסטי לחלוטין: לא מכיר Profile, לא קורא process.env, לא מכיר maxResults
import fetch from 'node-fetch';
import { getCleanMovieName } from './search.js';
import {
  isCached, isUsenet, isVIPSource, getSeeders, getSizeGB,
  getWeightTiers, getResWeight, getQualityScoreForPreSort
} from './utils.js';

/**
 * @param {string} type - movie/series/tv/channel
 * @param {string} idWithExt - tt123456.json או tt123456:1:2.json
 * @param {object} context - { timeoutMs, maxSizeGB, minSeedersUncached, addons, clientUA, clientIp }
 * @returns {Promise<Array>} מערך שלם של סטרימים תקינים, ממוינים לשלמות. ללא slice, ללא עיצוב.
 */
export async function fetchAndSortStreams(type, idWithExt, context) {
  ...
  // --- סינון לפי מגבלות גודל/seeders (עדיין דומיין-קריטי, לא תצוגתי) ---
  ...
  return safeStreams; // מערך שלם, ממוין, ללא slice, ללא עיצוב, ללא מחיקת שדות פנימיים
}
