...
import {
  streamConflictsWithExpectedTitles,
...
function finalizeStreams(rawStreams, { isEpisode, sizeLimitGB, minSeedersUncached, releaseYear, maxResults, type, expectedTitles = [] }) {
  ...
  // Drop hard title conflicts before ranking (additive searches can return same SxxExx, wrong show)
  let titleConflictDropped = 0;
  const titleFiltered = expectedTitles.length
    ? streams.filter(s => {
      if (streamConflictsWithExpectedTitles(s, expectedTitles)) {
        titleConflictDropped++;
        return false;
      }
      return true;
    })
    : streams;
  if (titleConflictDropped > 0) {
    console.log(
      `[ESAY STREAM] 🚫 Dropped ${titleConflictDropped} wrong-title stream(s) (release name ≠ ${expectedTitles.slice(0, 2).map(t => `"${t}"`).join('/ ')})`
    );
  }
...
export async function fetchAndSortStreams(type, idWithExt, context) {
  ...
  if (extraFanout.length > 0) {
    const host = (u) => {
      try {
        return new URL(u.replace(/\/manifest\.json$/i, '').replace(/\/$/, '')).hostname;
      } catch {
        return String(u).slice(0, 40);
      }
    };
    console.log(
      `[ID-RESOLVE] extra fan-out (additive, capped): ${extraFanout.map(p => `${p.extraId}@${host(p.url)}`).join(', ')}`
    );
  ...
  const aliasFanout = planAliasTextFanout(addons, resolvedCtx, titles);
  if (aliasFanout.length > 0) {
    ...
    console.log(
      `[ID-RESOLVE] alias text search (additive): ${uniqueTitles.map(t => `"${t}"`).join(', ')}` +
        ` on ${[...new Set(aliasFanout.map(p => host(p.url)))].join(', ')}`
    );
