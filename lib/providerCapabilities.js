...
/**
 * MediaFusion is the only family allowed to mint the Telegram/Your-Media VIP
 * signal, and only for single-hop direct links that are not IP-locked.
 */
function mediafusionVipRule({ text, isSingleHopHttp, isIpLocked }) {
  if (!isSingleHopHttp || isIpLocked) return false;
  return /\btelegram[\s._-]?bot\b|your[\s._-]?media\b/i.test(text);
}
