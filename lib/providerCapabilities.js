 ...
 * Matches "telegram_bot", "telegram-bot", "telegram bot", and "your media" variants.
 * Restricted to plain single-hop HTTP links that are not IP-locked (ElfCache).
 */
const TEXT_PREFIX_VIP_RE = /\btelegram[\s._-]?bot\b|your[\s._-]?media\b/i;

function textPrefixVipRule({ text, isSingleHopHttp, isIpLocked }) {
  if (!isSingleHopHttp || isIpLocked) return false;
  return TEXT_PREFIX_VIP_RE.test(String(text || ''));
}

/**
 * MediaFusion shares the global telegram_bot / your-media text-prefix VIP rule.
 */
function mediafusionVipRule(context) {
  return textPrefixVipRule(context);
}
...
/** Evaluate VIP: host-authoritative providers, then global telegram_bot prefix. */
export function evaluateVip(provider, context) {
  if (textPrefixVipRule(context)) return true;
